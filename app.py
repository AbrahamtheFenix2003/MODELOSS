...
def load_models():
    ...
    for name, p in paths.items():
        try:
            ...
            print(f"Advertencia: No se pudo cargar el modelo {p}.")
...
# Función para el reporte de EVALUACIÓN COMPLETA (la original)
def generate_evaluation_report(metrics, preds_dict, y_true, class_names):
    ...
    pdf.cell(0, 20, clean_text_for_pdf("REPORTE COMPARATIVO DE MODELOS"), ln=True, align="C")
    pdf.ln(5)
    pdf.set_font("Arial", "B", 16)
    pdf.cell(0, 10, clean_text_for_pdf("Clasificacion Cervical (Herlev Dataset)"), ln=True, align="C")
    pdf.ln(10)
    pdf.set_font("Arial", size=12)
    current_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    pdf.cell(0, 8, clean_text_for_pdf(f"Fecha de generacion: {current_time}"), ln=True, align="C")
    pdf.cell(0, 8, clean_text_for_pdf(f"Total de imagenes evaluadas: {len(y_true)}"), ln=True, align="C")
    clean_class_names = [clean_text_for_pdf(name) for name in class_names]
    pdf.cell(0, 8, clean_text_for_pdf(f"Clases: {', '.join(clean_class_names)}"), ln=True, align="C")

    # RESUMEN EJECUTIVO
    pdf.ln(15)
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 8, clean_text_for_pdf("RESUMEN EJECUTIVO"), ln=True)
    pdf.ln(5)
    best_model = max(metrics.items(), key=lambda x: x[1]['Accuracy'])
    best_model_name = clean_text_for_pdf(best_model[0])
    best_accuracy = safe_format_number(best_model[1]['Accuracy'])
    pdf.set_font("Arial", size=11)
    summary_text = f"El modelo con mejor rendimiento es {best_model_name} con una precision de {best_accuracy}."
    ...
    pdf.add_page()
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 8, clean_text_for_pdf("METRICAS GENERALES"), ln=True)
    pdf.ln(5)
    pdf.set_font("Arial", "B", 10)
    headers = ["Modelo", "Accuracy", "Recall", "F1-Score"]
    col_widths = [50, 30, 30, 30]
    for i, header in enumerate(headers):
        pdf.cell(col_widths[i], 8, clean_text_for_pdf(header), border=1, align="C")
    pdf.ln()
    pdf.set_font("Arial", size=10)
    for name, m in metrics.items():
        clean_name = clean_text_for_pdf(name)
        accuracy = safe_format_number(m['Accuracy'])
        recall = safe_format_number(m['Recall'])
        f1 = safe_format_number(m['F1-Score'])
        pdf.cell(col_widths[0], 8, clean_name, border=1, align="C")
        pdf.cell(col_widths[1], 8, accuracy, border=1, align="C")
        pdf.cell(col_widths[2], 8, recall, border=1, align="C")
        pdf.cell(col_widths[3], 8, f1, border=1, align="C")
        pdf.ln()
    ...
    for name, mdl in models.items():
        ...
        pdf.set_font("Arial", "B", 16)
        clean_name = clean_text_for_pdf(name)
        pdf.cell(0, 10, clean_text_for_pdf(f"ANALISIS DETALLADO: {clean_name}"), ln=True)
        pdf.ln(5)
        
        # Reporte de clasificación
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 8, clean_text_for_pdf("Reporte de Clasificacion:"), ln=True)
        pdf.ln(3)
        y_pred = preds_dict[name]
        try:
            rpt = classification_report(y_true, y_pred, target_names=clean_class_names, digits=4)
            pdf.set_font("Courier", size=8)
            for line in rpt.split("\n"):
                if line.strip():
                    pdf.cell(0, 4, clean_text_for_pdf(line), ln=True)
        except Exception as e:
            pdf.set_font("Arial", size=10)
            pdf.cell(0, 6, clean_text_for_pdf(f"Error generando reporte: {str(e)}"), ln=True)
        
        # Matriz de confusión
        pdf.ln(5)
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 8, clean_text_for_pdf("Matriz de Confusion:"), ln=True)
        try:
            ...
            ax.set_xticks(np.arange(len(clean_class_names)))
            ax.set_yticks(np.arange(len(clean_class_names)))
            ax.set_xticklabels(clean_class_names, rotation=45, ha='right')
            ax.set_yticklabels(clean_class_names)
            plt.colorbar(im, ax=ax, shrink=0.8)
            ax.set_title(f'Matriz de Confusion - {clean_name}', fontsize=14, fontweight='bold')
            ...
            pdf.set_font("Arial", size=10)
            pdf.cell(0, 6, clean_text_for_pdf(f"Error generando matriz: {str(e)}"), ln=True)
...
# Función para el reporte de UNA SOLA PREDICCIÓN
...
# Función para el reporte de MÚLTIPLES PREDICCIONES
...
if mode == "Clasificar con un Modelo":
    ...
    if not st.session_state.evaluation_completed:
        # CORRECCIÓN: Comprobar el tamaño del array
        if y_true.size == 0:
    ...
    if st.session_state.evaluation_completed and st.session_state.metrics:
        ...
        if st.button("📥 Generar y Descargar PDF de Evaluación", type="primary"):
            with st.spinner("Generando reporte PDF..."):
                pdf_buffer = generate_evaluation_report(
            ...
            if pdf_buffer is not None:
                st.success("✅ PDF de evaluación generado!")
                st.download_button(
                    label="⬇️ Descargar Reporte de Evaluación",
                    data=pdf_buffer,
                    file_name=f"reporte_evaluacion_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                    mime="application/pdf"
                )

    # CORRECCIÓN: Comprobar el tamaño del array
    elif not st.session_state.evaluation_completed and y_true.size > 0:
